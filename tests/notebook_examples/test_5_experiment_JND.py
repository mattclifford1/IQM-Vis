...
def build_exp(build_IQM_Vis):
    ...
    qtbotbis.mouseClick(
        test_window.window.widget_controls['button']['launch_exp_JND'], QtCore.Qt.MouseButton.LeftButton)
    QtTest.QTest.qWait(500)
...
def test_experiment_runs(build_exp):
    ...
    # test launched
    assert test_window.window.experiment_JND.experiments_tab.currentIndex() == 0

    # Start experiment
    qtbotbis.mouseClick(
        test_window.window.experiment_JND.widget_experiments['setup']['start_button'],
        QtCore.Qt.MouseButton.LeftButton)
    assert test_window.window.experiment_JND.experiments_tab.currentIndex() == 1

    # Start
    qtbotbis.mouseClick(
        test_window.window.experiment_JND.widget_experiments['preamble']['start_button'],
        QtCore.Qt.MouseButton.LeftButton)
    assert test_window.window.experiment_JND.experiments_tab.currentIndex() == 2


    # run through experiment
    i = 0
    while test_window.window.experiment_JND.experiments_tab.currentIndex() == 2 and i < 200:
        if i % 2 == 0:
            qtbotbis.mouseClick(
                test_window.window.experiment_JND.widget_experiments['exp']['same_button'], QtCore.Qt.MouseButton.LeftButton)
        else:
            qtbotbis.mouseClick(
                test_window.window.experiment_JND.widget_experiments['exp']['diff_button'], QtCore.Qt.MouseButton.LeftButton)
    ...
    # check finished
    QtTest.QTest.qWait(500)
    assert test_window.window.experiment_JND.experiments_tab.currentIndex() == 3
    ...
    qtbotbis.mouseClick(
        test_window.window.experiment_JND.widget_experiments['final']['quit_button'], QtCore.Qt.MouseButton.LeftButton)
