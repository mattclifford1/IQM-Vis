...
def test_change_graphs(build_IQM_Vis):
    ...
    # Go to the range tab
    test_window.window.tabs['graph'].setCurrentIndex(2)

    # cycle the metric graphs forwards
    for _ in range(5):
        button = test_window.window.widget_controls['button']['next_metric_graph']
        qtbotbis.mouseClick(button, QtCore.Qt.MouseButton.LeftButton)
        QtTest.QTest.qWait(500)
    # check not crashed
    assert test_window.showing == True

    #  cycle the metric graphs backwards
    for _ in range(5):
        button = test_window.window.widget_controls['button']['prev_metric_graph']
        qtbotbis.mouseClick(button, QtCore.Qt.MouseButton.LeftButton)
        QtTest.QTest.qWait(500)
