    ...
    def get_metrics_over_range(self):
        ...
        for im_pair in self.im_pair_names:
            ...
            for metric in self.metrics_dict.keys():
                results[str(im_pair)][metric] = {}
                for trans in self.sliders.keys():
                    results[str(im_pair)][metric][trans] = []
            ...
            for curr_trans in self.sliders.keys():
                for trans_value in self.sliders[curr_trans]['values']:
                    ...
                    for metric in self.metrics_dict.keys():
                        ...
                        results[str(im_pair)][metric][curr_trans].append(float(metric_score))
        self.plot_metrics_graphs(results)

    def plot_metrics_graphs(self, results):
        # plot
        for im_pair in self.im_pair_names:
            ...
            for metric in self.metrics_dict.keys():
                ...
                for trans in self.sliders.keys():
                    ...
                    mean_value.append(np.mean(results[str(im_pair)][metric][trans]))
                    # std_value.append(np.std(results[str(im_pair)][metric][trans]))
